...
const lessonMeta=document.body.dataset.lesson;if(lessonMeta){storage.set('lastLesson',lessonMeta)}
const resume=$('#resumeBox');if(resume){const d=safeParse(storage.get('lastLesson'),null);if(d&&d.url&&d.title&&d.author){resume.innerHTML=`<a class=btn href="${d.url}">Continua: ${d.title} · ${d.author}</a>`}}
